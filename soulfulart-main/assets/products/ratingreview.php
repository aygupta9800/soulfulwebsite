...
<?php
  if(basename($_SERVER['PHP_SELF'])=="ratingreview.php")
   {
    // echo "wats thsi";
      $pname=  basename($_SERVER['HTTP_REFERER']);
      echo $pname;
      $productname=explode(".",$pname)[0];        
  }else{
    // echo "yes";
    ...
    {       
        $productname=explode(".",$page)[0];       
    }
  }
    $userid=$_COOKIE["userid"];
    //echo $productname;
    $result = $conn->query("SELECT * FROM sys.products where name = '$productname';");
